%
% Figure 4 - The transmit power needed to achieve the rate R¡Â in the scenario
%            shown in Fig. 3, as a function of the distance d1
% ref. E. Bj?rnson, ?. ?zdogan and E. G. Larsson, "Intelligent Reflecting Surface 
...
noiseFiguredB = 10;
%
% compute the noise power in dBm, £m^2
%
...
for i = 1:length(Rbar)
    ...
    for k = 1:length(d1range)
        ...
        %
        betaSR = pathloss_3GPP_LOS(d_SR, fc) * Gs * Gr;  % £]_sr
        betaRD = pathloss_3GPP_LOS(d_RD, fc) * Gr * Gd;  % £]_rd
        betaSD = pathloss_3GPP_NLOS(d_SD, fc) * Gs * Gd; % £]_sd
        betaIRS = betaSR * betaRD;  % £]_IRS = £]_sr * £]_rd
        %
        % compute the transmit power in mW in the SISO case, using Eq.(11)
        %
        % p_SISO = (2^R¡Â - 1) * £m^2 / £]_sd;
        P_SISO(k) = SINR * sigma2 / betaSD;
        %
        % compute the transmit power in mW in the IRS case, using Eq.(12)
        %
        % p_IRS(N) = (2^R¡Â - 1) * £m^2 / (¡Ô£]_sd + N*£\*¡Ô£]_IRS)^2;
